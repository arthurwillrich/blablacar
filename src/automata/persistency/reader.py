...
def new_read_fa_from(filepath: str) -> FiniteAutomata:
    ...
    fa = FiniteAutomata()
    states = [str(x) for x in list(range(int(entry[0])))]
    fa.states = {State(s) for s in states}
    fa.initial_state = State(entry[1])
    fs = entry[2].split(',')
    fa.final_states = {State(s) for s in fs}
    fa.alphabet = entry[3].split(',')
    ...
    for i in entry[4:]:
        transic = i.split(",")
        src = __state_cache[transic[0]]
        input_symbol = transic[1]
        dst = search_states(transic[2].split('-'))
